    ...
    <div>
      ...
      <div class="bg-white rounded-xl shadow-md px-5">
        ...
        <table class="w-full table-dashboard text-sm border-b border-gray-200">
          ...
          <tbody>
            <tr v-for="(item, index) in filteredVerifikasi" :key="item.id"
              ...
              <td class="p-3 text-left">
                <button @click="router.push({ path: `/verifikasi/edit/${item.id}`, query: { page: currentPage.value } })"
...
select option:hover { background: #2563eb; color: white; }
select option:checked { background: #2563eb; color: white; }
</style>